...
def create_embedding(args: argparse.Namespace, G: nx.classes.graph.Graph) -> None:
    ...
    if args.method == 'node2vec_snap':
        ...
        pathi = pathlib.Path(__file__).parent.parent.absolute()

        print(pathi)
        arguments = ['--format', 'gpickle', '--input', str(pathi / args.input), '--number-walks', str(args.num_walks),
              '--representation-size', str(args.dimensions), '--walk-length', str(args.walk_length),
              '--window-size', str(args.window_size), '--workers', str(args.workers),
              '--seed', str(args.seed), '--output', str(pathi / args.output)]
        print(arguments)
        main(arguments)
        """
        arguments = ['--format', 'gpickle', '--input', {args.input}, '--number-walks', {args.num_walks},
              '--representation-size', {args.dimensions}, '--walk-length', {args.walk_length},
              '--window-size', {args.window_size}, '--workers', {args.workers},
